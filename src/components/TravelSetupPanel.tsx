...
import { CarIcon, CheckIcon, HotelIcon, RouteIcon } from './Icons'
...
const draftKey = (appointmentId: string) => `routine-assist-travel-setup:${appointmentId}`
...
export function TravelSetupPanel({ appointment, onFinish }: { appointment: Appointment; onFinish: () => void }) {
  ...
  const [title, setTitle] = useState(`Viagem ${appointment.city || appointment.client}`)
  ...
  useEffect(() => {
    try {
      ...
      if (!raw || linkedTrip) return
      const draft = JSON.parse(raw)
      setMode(draft.mode || 'choose')
      setTitle(draft.title || `Viagem ${appointment.city || appointment.client}`)
      setOrigin(draft.origin || 'Uberaba/MG')
  ...
  useEffect(() => {
    if (linkedTrip) setMode('done')
  }, [linkedTrip])

  useEffect(() => {
    if (linkedTrip || mode === 'done') return
    try { localStorage.setItem(draftKey(appointment.id), JSON.stringify({ mode, title, origin, start, end, hotelRequired, vehicleRequired, tripId })) } catch { /* noop */ }
  }, [appointment.id, linkedTrip, mode, title, origin, start, end, hotelRequired, vehicleRequired, tripId])

  const sortedTrips = useMemo(() => [...trips].sort((a,b) => a.start.localeCompare(b.start)), [trips])
  ...
  const handleCreate = async () => {
    ...
    try {
      const id = await createTrip({
        title: title.trim(), origin: origin.trim() || undefined, start, end,
        hotelRequired, vehicleRequired, appointmentIds: [appointment.id],
      })
      finishWithTrip(id)
  ...
  const handleLink = async () => {
    ...
    setBusy(true); setError(null)
    try {
      await linkAppointmentsToTrip(tripId, [appointment.id])
      finishWithTrip(tripId)
  ...
  if (mode === 'done') {
    ...
    return <div className="travel-setup success-stage">
      <div className="travel-success-head"><span className="section-icon neutral"><CheckIcon/></span><div><strong>{trip ? 'Viagem organizada' : 'Atendimento vinculado'}</strong><p>{trip ? `${trip.title} • ${formatDateRange(trip.start, trip.end)}` : 'O atendimento já faz parte de uma viagem.'}</p></div></div>
      <div className="next-logistics-grid">
        <div><HotelIcon/><span><strong>Hospedagem</strong><small>{trip?.hotelRequired === false ? 'Não necessária' : 'Cadastrar ou confirmar hotel'}</small></span></div>
        <div><CarIcon/><span><strong>Veículo</strong><small>{trip?.vehicleRequired === false ? 'Não necessário' : 'Solicitar pelo Forms e acompanhar'}</small></span></div>
      </div>
  ...
  if (mode === 'create') return <div className="travel-setup">
    <div className="travel-stage-head"><button className="text-back" onClick={()=>setMode('choose')}>← Voltar</button><div><span className="eyebrow">Etapa 2 de 2</span><h3>Criar viagem</h3><p>O atendimento já será vinculado automaticamente.</p></div></div>
    ...
    <p className="field-help">As datas começaram iguais ao atendimento. Ajuste a saída e o retorno conforme sua logística.</p>
    <div className="travel-options compact-options">
      <label className="confirm-row"><input type="checkbox" checked={hotelRequired} onChange={e=>setHotelRequired(e.target.checked)}/><span><strong>Precisa de hotel</strong><small>Gera alertas até a hospedagem estar organizada.</small></span></label>
      <label className="confirm-row"><input type="checkbox" checked={vehicleRequired} onChange={e=>setVehicleRequired(e.target.checked)}/><span><strong>Precisa de veículo</strong><small>Gera alerta para solicitar e confirmar a reserva.</small></span></label>
    </div>
    {error && <div className="auth-message error modal-error">{error}</div>}
    <div className="modal-actions"><button className="ghost" onClick={()=>setMode('choose')}>Voltar</button><button className="primary" disabled={!title.trim()||!start||!end||busy} onClick={()=>void handleCreate()}><RouteIcon/> {busy?'Criando...':'Criar e vincular'}</button></div>
  </div>

  if (mode === 'link') return <div className="travel-setup">
    <div className="travel-stage-head"><button className="text-back" onClick={()=>setMode('choose')}>← Voltar</button><div><span className="eyebrow">Etapa 2 de 2</span><h3>Vincular a uma viagem</h3><p>Se o atendimento ampliar o período, o Routine ajusta automaticamente a viagem.</p></div></div>
    <div className="existing-trip-list">
      ...
      {sortedTrips.map(t => {
        const contains = t.start <= appointment.start && t.end >= appointment.end
        return <button type="button" key={t.id} className={`existing-trip-choice ${tripId===t.id?'selected':''}`} onClick={()=>setTripId(t.id)}>
          <span className="choice-check">{tripId===t.id?<CheckIcon/>:<RouteIcon/>}</span>
          <span className="grow"><strong>{t.title}</strong><small>{formatDateRange(t.start,t.end)} • {t.appointments.length} atendimento{t.appointments.length===1?'':'s'}</small></span>
          <span className={`compat-badge ${contains?'ok':'adjust'}`}>{contains?'Período compatível':'Período será ajustado'}</span>
        </button>
      })}
    </div>
    {error && <div className="auth-message error modal-error">{error}</div>}
