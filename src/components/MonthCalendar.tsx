...
import { TripLogisticsIcons, TripStatusBadge, tripDone, tripReadinessStatus } from './CalendarTripVisuals'
...
export function MonthCalendar({ month, trips, appointments, demands=[], holidays=[], onDaySelect }:{month:Date;trips:Trip[];appointments:Appointment[];demands?:Demand[];holidays?:Holiday[];onDaySelect?:(day:Date)=>void}){
  ...
  return <div className="month-calendar">
    ...
    {weeks.map((weekStart)=>{
      ...
      return <div className="month-week" key={weekStart.toISOString()} style={{gridTemplateRows:`36px repeat(${rows-1},58px)`}}>
        ...
        {weekTrips.map((t,i)=>{const key=tripCompanyKey(t,demands); const commercial=compactTripCommercials(t,demands,2); const done=tripDone(t,demands); const status=tripReadinessStatus(t); return <button type="button" className={`month-event trip month-event-button ${companyClass(key)} ${done?'calendar-event-done':''}`} key={t.id} onClick={()=>onDaySelect?.(isBefore(parseISO(t.start),weekStart)?weekStart:parseISO(t.start))} style={{gridColumn:clipPosition(t.start,t.end,weekStart),gridRow:i+2}}><span className="month-event-main"><b>{companyLabel(key)}</b><strong>{compactTripStops(t,2)}</strong><TripLogisticsIcons trip={t} done={done}/></span><small className="month-event-detail"><span>{compactTripClients(t,2)}</span>{commercial&&<span>Comercial: {commercial}</span>}</small><span className="month-event-aux"><TripWeatherChip trip={t}/>{status.tone!=='ready'&&<TripStatusBadge trip={t}/>}</span></button>})}
