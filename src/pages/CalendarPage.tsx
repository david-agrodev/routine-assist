...
export function CalendarPage(){
  ...
  const linkedAppointments = useMemo(()=>{
    ...
    trips.forEach(trip => trip.appointments.forEach(appointment => {
      ids.add(appointment.id)
      if (appointment.demandId) demandIds.add(appointment.demandId)
    }))
    return { ids, demandIds }
  },[trips])
  const standaloneAppointments = useMemo(()=>appointments.filter(appointment => {
    if (linkedAppointments.ids.has(appointment.id)) return false
    if (appointment.demandId && linkedAppointments.demandIds.has(appointment.demandId)) return false
    return true
